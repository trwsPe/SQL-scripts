create database Books_reviews;
use Books_reviews;

-- TABLES CREATION 

create table authors(
author_id float primary key,
author nvarchar(200)
);

create table reviews(
review_id float primary key,
score float,
summary nvarchar(255),
text nvarchar(2100),
helpfulness float,
confidence float
);

create table details(
isbn nvarchar(10) primary key,
title nvarchar(255),
description nvarchar(1100),
publisher nvarchar(255),
published_year int,
published_month int
);


create table categories(
category_id float primary key,
category nvarchar(100)
);

create table books(
user_id nvarchar(25),
review_id float,
category_id float,
author_id float,
isbn nvarchar(10),
ratings_count float,
primary key (user_id,review_id,category_id,author_id,isbn),
foreign key (review_id) references reviews(review_id),
foreign key (category_id) references categories(category_id),
foreign key (author_id) references authors(author_id),
foreign key (isbn) references details(isbn),
);


-- DATA LOADING

insert into authors
select distinct author_id ,authors
	from merged_data;

insert into reviews
select distinct review_id, null , null ,null ,null, null
	from merged_data;

/* 
Because in the merged_data table there were multiple records for combinations of attributes that should be distinct in the reviews and details tables,
duplicate values were being created in the primary key. To address this issue, we first inserted only the IDs of these two tables using a SELECT DISTINCT statement,
and then used a MERGE statement to populate the remaining attributes.
*/

MERGE
INTO reviews AS target
USING (SELECT
        review_id,
        MAX(score) AS score,
        MAX(summary) AS summary,
        MAX(text) AS text,
        MAX(helpfulness) AS helpfulness,
        MAX(confidence) AS confidence
    FROM merged_data
    GROUP BY review_id
) AS source
ON target.review_id = source.review_id
WHEN MATCHED
  THEN UPDATE
      SET score = source.score
		 ,summary = source.summary
         ,text = source.text
		 ,helpfulness = source.helpfulness
		 ,confidence = source.confidence;


Insert into details
select distinct isbn, null , null ,null ,null, null
	from merged_data;

MERGE
INTO details AS target
USING (SELECT
        isbn,
        MAX(Title) AS Title,
        MAX(description) AS description,
        MAX(publisher) AS publisher,
        MAX(datepart(year,publishedDate)) AS year,
        MAX(datepart(month,publishedDate)) AS month
	   FROM merged_data
       GROUP BY isbn
) AS source
ON target.isbn = source.isbn
WHEN MATCHED
  THEN UPDATE
      SET Title = source.Title
		 ,description = source.description
         ,publisher = source.publisher
		 ,published_year = source.year
		 ,published_month = source.month;



insert into categories
select distinct category_id ,categories
	from merged_data;


insert into books
select distinct user_id ,review_id ,category_id,author_id,isbn,ratingscount
from merged_data;

